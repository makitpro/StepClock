...
/*This is the code for makit's STEP CLOCK
 ...
 In this version, you'll have to set the birthday day month and hour and
 a song will play. To stop the song, just reset the Arduino board wth the reset button.
 The song was extracted from:https://create.arduino.cc/projecthub/trduunze/piezo-happy-birthday-7ea362
 */
...
//initialize buzzer-anniversary variables
int buzzerPin=3; //pin of the buzzer
int length = 28; // the number of notes
char notes[] = "GGAGcB GGAGdc GGxecBA yyecdc";
int beats[] = { 2, 2, 8, 8, 8, 16, 1, 2, 2, 8, 8,8, 16, 1, 2,2,8,8,8,8,16, 1,2,2,8,8,8,16 };
int tempo = 150;

  //Set anniversary day and month, change it to match yours
int birthdayDay=11; //Set the anniversary day
int birthdayMonth=7; //Set the anniversary month in numbers
int birthdayHour=12;////Set the time at which the alarm will sound in 24h format
  ...
  if (++minutes >= 60) {
    ...
    moveMotorsToPosition();
     ...
     //Check for anniversary date
      if (days==birthdayDay && months==birthdayMonth && hours24==birthdayHour){
      while(1){
      happy_song();  
      }
      }
